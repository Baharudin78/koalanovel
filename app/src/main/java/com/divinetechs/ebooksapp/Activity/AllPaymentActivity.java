...
import com.divinetechs.ebooksapp.Model.PayTmModel.Paytm;
...
public class AllPaymentActivity extends AppCompatActivity implements PaymentResultListener, View.OnClickListener,
    ...
    LinearLayout lyToolbar, lyBack, lyInApp, lyRazorpay, lyPaypal, lyFlutterwave, lyPayumoney, lyPaytm, lyWalletBalance, lyPayNow, lyBottom;
    TextView txtToolbarTitle, txtWalletBalance, txtPayableAmount;
    CheckBox cbWallet;
    ...
    boolean flag = false, walletCheck = false, isPayWithWallet = false;
    String name = "", price = "", finalPaymentType = "", paymentType = "", currencyCode = "", shortDescription = "", itemID = "",
            paymentId = "", state = "1", createTime = "", authorId = "", TYPE = "", fixedWalletAmount = "", walletAmount = "", gatewayAmout = "", payWith = "";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        Utils.screenCapOff(AllPaymentActivity.this);
    ...
    private void makePaymentID() {
        if (TYPE.equalsIgnoreCase("Package")) {
            paymentId = itemID + price + "_" + prefManager.getLoginId();
        } else {
            paymentId = itemID + authorId + price + "_" + prefManager.getLoginId();
        }
    }
    ...
    private void Purchasebook() {
        ...
        if (TYPE.equalsIgnoreCase("Book")) {
            call = bookNPlayAPI.add_purchase(itemID, prefManager.getLoginId(), price, currencyCode, name, state, authorId, paymentId, finalPaymentType, walletAmount, gatewayAmout);

        } else if (TYPE.equalsIgnoreCase("Magazine")) {
            call = bookNPlayAPI.add_magazine_purchase(itemID, prefManager.getLoginId(), price, currencyCode, name, state, authorId, paymentId, finalPaymentType, walletAmount, gatewayAmout);

        } else if (TYPE.equalsIgnoreCase("Package")) {
            call = bookNPlayAPI.add_package_transaction("" + prefManager.getLoginId(), price, itemID, paymentId, state);
        }
    ...
    private void payUMoney() {
        ...
        builder.setAmount("" + gatewayAmout)
                ...
                .setUdf9("")
                .setUdf10("")
                .setIsDebug(Constant.PayU_isTest) // Integration environment - true (Debug)/ false(Production)
    ...
    private void generateCheckSum() {
        ...
        Call<PayTmModel> call = bookNPlayAPI.getPaymentToken(
                paytm.getmId(),
                paytm.getOrderId(),
                paytm.getCustId(),
                paytm.getChannelId(),
    ...
    public void onTransactionResponse(Bundle bundle) {
        ...
        if (bundle.getString("STATUS").equalsIgnoreCase("TXN_SUCCESS")) {
            Toasty.success(AllPaymentActivity.this, bundle.toString(), Toasty.LENGTH_LONG).show();
            String orderId = bundle.getString("ORDERID");
            Log.e("PaytmPayment", "orderId --> " + orderId);
            Purchasebook();
        } else {
